...
export type EmitTest = {
    $$type: "EmitTest";
    number: bigint;
    counter: bigint;
};

export function storeEmitTest(src: EmitTest) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeUint(1480778918, 32);
        b_0.storeUint(src.number, 32);
        b_0.storeUint(src.counter, 32);
    };
}

export function loadEmitTest(slice: Slice) {
    let sc_0 = slice;
    if (sc_0.loadUint(32) !== 1480778918) {
        throw Error("Invalid prefix");
    }
    let _number = sc_0.loadUintBig(32);
    let _counter = sc_0.loadUintBig(32);
    return { $$type: "EmitTest" as const, number: _number, counter: _counter };
}

function loadTupleEmitTest(source: TupleReader) {
    let _number = source.readBigNumber();
    let _counter = source.readBigNumber();
    return { $$type: "EmitTest" as const, number: _number, counter: _counter };
}

function loadGetterTupleEmitTest(source: TupleReader) {
    let _number = source.readBigNumber();
    let _counter = source.readBigNumber();
    return { $$type: "EmitTest" as const, number: _number, counter: _counter };
}

function storeTupleEmitTest(source: EmitTest) {
    let builder = new TupleBuilder();
...
function dictValueParserEmitTest(): DictionaryValue<EmitTest> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeEmitTest(src)).endCell());
        },
        parse: (src) => {
            return loadEmitTest(src.loadRef().beginParse());
        },
    };
}
...
export function loadMain$Data(slice: Slice) {
    ...
    let _counter = sc_0.loadUintBig(32);
    return { $$type: "Main$Data" as const, id: _id, counter: _counter };
}
...
async function Main_init(id: bigint) {
    const __code = Cell.fromBase64(
        "",
    );
    const __system = Cell.fromBase64(
        "",
    );
...
const Main_types: ABIType[] = [
    ...
    },
    {
        ...
        header: 1480778918,
        fields: [
            { name: "number", type: { kind: "simple", type: "uint", optional: false, format: 32 } },
            {
                name: "counter",
